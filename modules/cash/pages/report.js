...
module.exports = function account(webapp) {
    ...
    app.get(prefix + "/reports/dblflow", webapp.layout(), function (req, res, next) {
        report1(req, res, next, "dblflow");
    });
    ...
    function report1(req, res, next, type) {
        if (!req.query || !req.query.name) {
            ...
            if (type == 'pieflow')
                ...
                ct = 'Bar flow chart';
            else if (type == 'dblflow')
                ct = 'Double flow chart';
    ...
    function calculateGraphData(token, type, params, cb){
        ...
        async.waterfall([
            ...
            function(trns,cb1){
                (function (cb) {
                        ...
                        cashapi.getCmdtyPrice(token,tr.currency,{space:"ISO4217",id:params.reportCurrency},null,'safe',function(err,rate){
                            ...
                            _.forEach(tr.splits, function(split) {
                                ...
                                if (acs) {
                                    ...
                                    if (twoBars ? params.accType!='INCOME' && params.accType!='EXPENSE' : params.accType!=acs.type)
                                    // if (params.accType!=acs.type)
                                    // if (params.accType!='INCOME' && params.accType!='EXPENSE')
                                    {
                                        acs.summ  = 0;
                                        val = 0;
                                    }
                    ...
                    if(params.accLevel != 'All'){
                        async.series([
                            ...
                            function(cb2){
                                do {
                                    ...
                                    _.each(accKeys, function (item, id) {
                                        ...
                                        if (item.level<=params.accLevel)
                                            delete ids[id];
                                        if (twoBars ? item.type!='INCOME' && item.type!='EXPENSE' : item.type!=params.accType)
                                ...
                                accKeys = _(accKeys).reduce(function (memo, acc) {
                                    if ((twoBars ? acc.type == 'INCOME' || acc.type == 'EXPENSE' : acc.type == params.accType) || acc.expand)
                        ...
                        accKeys = _(accKeys).reduce(function (memo, acc) {
                            if (twoBars ? acc.type == 'INCOME' || acc.type == 'EXPENSE' : acc.type == params.accType)
            ...
            function(cb1){
                ...
                if (twoBars)
                {
                    var iacs_inc = _(accKeys).chain().map(function(acs)
                    {
                        return {_id: acs._id, summ: acs.summ, type: acs.type};
                    }).filter(function(acs)
                    {
                        return acs.type=='INCOME';
                    }).sortBy(function(acs)
                    {
                        return acs.summ;
                    }).last(params.maxAcc).reduce(function(memo,acs)
                    {
                        memo[acs._id] = 1;
                        return memo;
                    },{}).value();
                    var iacs_exp = _(accKeys).chain().map(function(acs)
                    {
                        return {_id: acs._id, summ: acs.summ, type: acs.type};
                    }).filter(function(acs)
                    {
                        return acs.type=='EXPENSE';
                    }).sortBy(function(acs)
                    {
                        return acs.summ;
                    }).last(params.maxAcc).reduce(function(memo,acs)
                ...
                var final = _(accKeys).reduce( function (memo, accKey) {
                    total += accKey.summ;
                    if (twoBars ? _(iacs_inc).has(accKey._id) || _(iacs_exp).has(accKey._id) : _(iacs).has(accKey._id))
                ...
                var report = _(final).reduce( function (memo,accKey) {
                    ...
                    if (periods){
                        if (twoBars)
                        {
                            obj = {name:accKey.name, data:_(accKey.periods).pluck('summ'), stack:accKey.type=='INCOME' ? 'Income' : 'Expense'};
                            if (accKey.type=='INCOME')
                            {
                                obj.data = _(obj.data).map(function(summ)
                                {
                                    return -summ;
                                });
                            }
                        } else {
                            obj = {name:accKey.name, data:_(accKey.periods).pluck('summ')};
                        }
                    } else {
